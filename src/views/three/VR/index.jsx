...
class VR extends Component {
  ...
  async init() {
    ...
    const camera = new THREE.PerspectiveCamera(50, this.state.WIDTH / this.state.HEIGHT, 0.1, 500)
    ...
    await setStateSync(this, { position, tangent, lookAt, velocity, prevTime, progress })
  ...
  renderVR = async () => {
    ...
    this.state.train.lookAt(this.state.lookAt.copy(this.state.position).sub(this.state.tangent))
  ...
  onWindowResize = async () => {
    ...
    if (VRcontainer) {
      await setStateSync(this, { WIDTH: VRcontainer.clientWidth, HEIGHT: VRcontainer.clientHeight })
